...
export function DateTicker({ currentIndex, labels, visible }: DateTickerProps) {
  ...
  const monthIndices = useMemo(() => {
    ...
    parsedLabels.forEach(
      (label: { month: string; day: string; full: string }, index: number) => {
  ...
  return (
    <motion.div
      className="overflow-hidden rounded-full bg-[color:var(--chart-tooltip-background)] px-4 py-1 text-[color:var(--chart-tooltip-foreground)] shadow-lg"
      layout
      transition={{
        ...
        <div className="flex items-center justify-center gap-1">
          ...
          <div className="relative h-6 overflow-hidden">
            <motion.div className="flex flex-col" style={{ y: monthY }}>
              {monthIndices.uniqueMonths.map((month: string) => (
                <div
